...
smoketests := check-addons check-basic check-byocri check-dualstack check-hacontrolplane check-install check-kine check-multicontroller check-network check-singlenode check-customports
.PHONY: $(smoketests)
$(smoketests): k0s
	$(MAKE) -C inttest $@
