...
func (d *dirWatch) runFSNotify(ctx context.Context) (err error) {
	...
	for {
		...
		case event := <-watcher.Events:
			...
			case event.Has(fsnotify.Remove):
				...
				d.handler.Handle(&Changed{name, PathGone})
			...
			case event.Has(fsnotify.Create), event.Has(fsnotify.Write), event.Has(fsnotify.Chmod):
				// Suppress events for the watched directory itself, as per package contract.
				if event.Name == d.path {
					break
				}

				d.handler.Handle(&Changed{name, PathInfoFunc(sync.OnceValues(func() (fs.FileInfo, error) {
					return os.Lstat(event.Name)
				}))})
		...
		case err := <-watcher.Errors:
			...
			case errors.Is(err, fsnotify.ErrEventOverflow):
				d.handler.Handle(&Desynced{Path: d.path})
			default:
