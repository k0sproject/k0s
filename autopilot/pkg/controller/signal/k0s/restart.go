...
import (
	...
	apsigpred "github.com/k0sproject/autopilot/pkg/controller/signal/common/predicate"
...
//
// Subsequent reconciliations will occur when the object is created, which presents
// an opportunity for this reconciler to determine the installed `k0s` version. If the
// version is not the expected version, the request is re-queued.
//
// If the installed `k0s` version is the version specified in the plan, the plan will
// move to 'Completed'.
func (r *restart) Reconcile(ctx context.Context, req cr.Request) (cr.Result, error) {
	...
	if k0sVersion == signalData.Command.Update.K0s.Version {
