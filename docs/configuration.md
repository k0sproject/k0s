...
The repository includes a JSON Schema at `schemas/k0s.json` for field completion,
descriptions, and structural validation of `k0s.yaml`. Use the schema from the same
k0s release tag as your installation; the schema on `main` follows development.

For example, copy `schemas/k0s.json` from a k0s checkout alongside your `k0s.yaml`
and add this comment at the top of the YAML file:

```yaml
# yaml-language-server: $schema=./k0s.json
```

This works with the [YAML extension for VS Code](https://github.com/redhat-developer/vscode-yaml)
and other editors using [yaml-language-server](https://github.com/redhat-developer/yaml-language-server).
...
Contributors can regenerate the schema with `make config-schema` (or
`make config-schema DOCKER=` with a local Go toolchain). It is also regenerated by
