...
`spec.patches` lets you customize the Kubernetes resources that k0s generates
for its built-in components before they are written and applied. This is an
escape hatch for adjustments k0s does not expose as dedicated config options.

Patching is currently supported for the resources generated by these
components: CoreDNS, kube-proxy, metrics-server, kube-router and Calico.

Each patch selects a target resource by `kind` and `name` (optionally narrowed
