...
- docker.io/k0sproject/k0s:v1.26.11-k0s.0
- ghcr.io/k0sproject/k0s:v1.26.11-k0s.0

**Note:** Due to Docker Hub tag validation scheme, we have to use `-` as the k0s version separator instead of the usual `+`. So for example k0s version `v1.26.11+k0s.0` is tagged as `docker.io/k0sproject/k0s:v1.26.11-k0s.0`.
...
```sh
docker run -d --name k0s --hostname k0s --privileged -v /var/lib/k0s -p 6443:6443 docker.io/k0sproject/k0s:v1.26.11-k0s.0
```
...
2. Run the container to create and join the new worker:
    ...
    ```sh
    docker run -d --name k0s-worker1 --hostname k0s-worker1 --privileged -v /var/lib/k0s docker.io/k0sproject/k0s:v1.26.11-k0s.0 k0s worker $token
    ```
...
services:
  k0s:
    container_name: k0s
    image: docker.io/k0sproject/k0s:v1.26.11-k0s.0
