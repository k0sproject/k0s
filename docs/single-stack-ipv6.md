...
K0s introduced experimental IPv6 single-stack networking, allowing your cluster to operate
exclusively with IPv6 addresses. IPv6 single stack is currently in alpha state and requires
to enable explicitly the [IPv6SingleStack feature gate](feature-gates.md) on every controller.

IPv6 single-stack is fully compatible with both [node-local load balancing](nllb.md) and
[control plane load balancing (CPLB)](cplb.md).
