...
import (
	...
	"github.com/k0sproject/k0s/internal/autopilot/pkg/templatewriter"
)
...
// Mkdir creates a directory with a specific permission
func (s *FootlooseSuite) Mkdir(node, path, perms string) {
	ssh, err := s.SSH(node)
	s.Require().NoError(err)
	defer ssh.Disconnect()

	_, err = ssh.ExecWithOutput(fmt.Sprintf("mkdir -p '%s' && chmod %s '%s'", path, perms, path))
