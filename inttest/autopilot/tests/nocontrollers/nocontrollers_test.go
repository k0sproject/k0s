...
type nocontrollersSuite struct {
...
const network = "nocontrollersnet"

// SetupSuite creates the required network before starting footloose.
func (s *nocontrollersSuite) SetupSuite() {
...
func (s *nocontrollersSuite) TearDownSuite() {
...
func (s *nocontrollersSuite) SetupTest() {
	...
	for idx := 0; idx < s.FootlooseSuite.ControllerCount; idx++ {
		...
		s.Require().NoError(s.InitControllerAutopilot(idx, "--kubeconfig=/var/lib/k0s/pki/admin.conf", "--mode=controller", "--exclude-from-plans=controller"))
...
// TestApply applies a well-formed `plan` yaml, and asserts that all of the correct values across
// different objects + controllers are correct. Since `autopilot` was run with `--exclude-from-plans=controller`,
// this will result in this plan failing with `PlanRestricted`
func (s *nocontrollersSuite) TestApply() {
	planTemplate := `
...
`
	...
	plan, err := apcomm.WaitForPlanByName(context.TODO(), client, apconst.AutopilotName, 10*time.Minute, func(obj interface{}) bool {
		if plan, ok := obj.(*apv1beta2.Plan); ok {
			return plan.Status.State == appc.PlanRestricted
		}
	...
	s.NoError(err)
	s.Equal(appc.PlanRestricted, plan.Status.State)
}

// TestNoControllersSuite sets up a suite using 3 controllers for quorum, and runs various
// autopilot upgrade scenarios against them.
func TestNoControllersSuite(t *testing.T) {
	suite.Run(t, &nocontrollersSuite{
		apitcomm.FootlooseSuite{
			ControllerCount:    3,
			WorkerCount:        3,
			WithLB:             true,
