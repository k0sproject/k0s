...
type CPLBUserSpaceExtAddrSuite struct {
...
func (s *CPLBUserSpaceExtAddrSuite) TestK0sGetsUp() {
	lb := s.getLBAddress()
	ctx := s.Context()
	...
	for idx := range s.BootlooseSuite.ControllerCount {
		...
		s.Require().NoError(s.InitController(idx, "--config=/tmp/k0s.yaml", "--enable-worker", "--disable-components=metrics-server,endpoint-reconciler", joinToken))
		...
		if idx == 0 {
			...
			// Wait for node to be ready before proceeding with the second controller
...
func (s *CPLBUserSpaceExtAddrSuite) getLBAddress() string {
...
func (s *CPLBUserSpaceExtAddrSuite) checkDummy(ctx context.Context, node string) {
...
func (s *CPLBUserSpaceExtAddrSuite) hasVIP(ctx context.Context, node string, vip string) bool {
	ssh, err := s.SSH(ctx, node)
	s.Require().NoError(err)
	defer ssh.Disconnect()
...
func TestCPLBUserSpaceExtAddrSuite(t *testing.T) {
	suite.Run(t, &CPLBUserSpaceExtAddrSuite{
		common.BootlooseSuite{
			ControllerCount: 3,
			WorkerCount:     1,
		},
