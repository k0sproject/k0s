...
func (s *customPortsSuite) TestControllerJoinsWithCustomPort() {
	...
	s.Require().NoError(common.WaitForKubeRouterReady(kc), "calico did not start")
	s.T().Log("waiting to see konnectivity-agent pods ready")
	s.Require().NoError(common.WaitForDaemonSet(kc, "konnectivity-agent"), "konnectivity-agent did not start")
