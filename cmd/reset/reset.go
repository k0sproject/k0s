...
func (c *CmdOpts) reset() error {
	...
	if k0sStatus != nil && k0sStatus.Pid != 0 {
		c.Logger.Fatal("k0s seems to be running! please stop k0s before reset.")
	}
	...
	c.Logger.Info("k0s cleanup operations done. To ensure a full reset, a node reboot is recommended.")
	return err
}
