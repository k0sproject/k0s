...
func (c *Certificates) Init(ctx context.Context) error {
	...
	if err := c.CertManager.EnsureCA("ca", "kubernetes-ca", c.ClusterSpec.API.CAExpiry); err != nil {
		return err
	}
	...
	eg.Go(func() error {
		// Front proxy CA
		if err := c.CertManager.EnsureCA("front-proxy-ca", "kubernetes-front-proxy-ca", c.ClusterSpec.API.CAExpiry); err != nil {
			return err
		}
		...
		_, err := c.CertManager.EnsureCertificate(proxyClientReq, apiServerUID, c.ClusterSpec.API.CertExpiry)
	...
	eg.Go(func() error {
		...
		adminCert, err := c.CertManager.EnsureCertificate(adminReq, users.RootUID, c.ClusterSpec.API.CertExpiry)
		if err != nil {
			return err
		}
	...
	eg.Go(func() error {
		...
		konnectivityCert, err := c.CertManager.EnsureCertificate(konnectivityReq, uid, c.ClusterSpec.API.CertExpiry)
		if err != nil {
			return err
		}
	...
	eg.Go(func() error {
		...
		ccmCert, err := c.CertManager.EnsureCertificate(ccmReq, apiServerUID, c.ClusterSpec.API.CertExpiry)
		if err != nil {
			return err
		}
	...
	eg.Go(func() error {
		...
		schedulerCert, err := c.CertManager.EnsureCertificate(schedulerReq, uid, c.ClusterSpec.API.CertExpiry)
		if err != nil {
			return err
		}
	...
	eg.Go(func() error {
		...
		_, err := c.CertManager.EnsureCertificate(kubeletClientReq, apiServerUID, c.ClusterSpec.API.CertExpiry)
		return err
	})
	...
	eg.Go(func() error {
		...
		_, err = c.CertManager.EnsureCertificate(serverReq, apiServerUID, c.ClusterSpec.API.CertExpiry)
		return err
	})
	...
	eg.Go(func() error {
		...
		_, err := c.CertManager.EnsureCertificate(apiReq, apiServerUID, c.ClusterSpec.API.CertExpiry)
		return err
	})
