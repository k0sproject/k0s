...
func (hc *Commands) AddRepository(repoCfg v1beta1.Repository) error {
	if repoCfg.IsOCI() {
		return hc.registryManager.AddRegistry(repoCfg)
	}
...
func (hc *Commands) InstallChart(ctx context.Context, chartName string, version string, releaseName string, namespace string, values map[string]interface{}, timeout time.Duration) (*release.Release, error) {
	...
	registryClient, err := hc.registryManager.GetRegistryClient(chartName)
	if err != nil {
	...
	chartDir, err := hc.locateChart(chartName, version, registryClient)
	if err != nil {
		return nil, err
	}
	...
	if err := hc.downloadDependencies(loadedChart, chartDir, registryClient); err != nil {
		return nil, err
	}
