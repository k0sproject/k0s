...
func (hc *Commands) downloadDependencies(chart *chartv2.Chart, chartPath string, registryClient *registry.Client) error {
	ac, err := ci.NewAccessor(chart)
	if err != nil {
		return err
	}
...
func (hc *Commands) resolveRegistryClient(chartName string) (*registry.Client, error) {
	...
	if client == nil && registry.IsOCI(chartName) {
		client, err = registry.NewClient(registry.ClientOptWriter(os.Stdout))
		if err != nil {
			return nil, fmt.Errorf("can't create anonymous registry client for chart `%s`: %w", chartName, err)
		}
	}
	return client, nil
}
...
func (hc *Commands) isInstallable(chart *chartv2.Chart) bool {
...
func (hc *Commands) UninstallRelease(ctx context.Context, releaseName, namespace string) error {
	...
	helmAction.WaitStrategy = kube.LegacyStrategy
