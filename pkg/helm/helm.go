...
func (hc *Commands) getActionCfg(ctx context.Context, namespace string) (*action.Configuration, error) {
	...
	driverlogger := slog.New(sloghook.NewHandler(log.WithField("helm", "driver").Logger, nil))
	driver.SetLogger(driverlogger.Handler())
	...
	actionlogger := slog.New(sloghook.NewHandler(log.WithField("helm", "action").Logger, nil))
	actionCfg.SetLogger(actionlogger.Handler())
