...
func TestCRIConfigurer_ResolveImports(t *testing.T) {
	t.Run("should have single import for CRI if there's nothing in imports dir", func(t *testing.T) {
		testLoadPath := path.Join(t.TempDir(), "*.toml")
		criRuntimePath := path.Join(t.TempDir(), "cri.toml")
		c := CRIConfigurer{
			loadPath:       testLoadPath,
			criRuntimePath: criRuntimePath,
			log:            logrus.New().WithField("test", t.Name()),
		}
		imports, err := c.ResolveImports()
		require.NoError(t, err)
	...
	t.Run("should have single import for all CRI configs", func(t *testing.T) {
		tmp := t.TempDir()
		testLoadPath := path.Join(tmp, "*.toml")
		criRuntimePath := path.Join(t.TempDir(), "cri.toml")
		criRuntimeConfig := `
[plugins."io.containerd.grpc.v1.cri".registry.mirrors]
  [plugins."io.containerd.grpc.v1.cri".registry.mirrors."docker.io"]
`
		err := os.WriteFile(path.Join(tmp, "foo.toml"), []byte(criRuntimeConfig), 0644)
		require.NoError(t, err)
		c := CRIConfigurer{
			loadPath:       testLoadPath,
			criRuntimePath: criRuntimePath,
...
version = 2
`
		nonCriConfigPath := path.Join(tmp, "foo.toml")
