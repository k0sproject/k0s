...
func (ec *ExtensionsController) reconcileStack(ctx context.Context, charts k0sv1beta1.ChartsSettings) bool {
	...
	for content, err := range eachCRD(HelmExtensionStackName) {
		if err != nil {
	...
	for _, chart := range charts {
		if err := chartTemplate.Execute(&stack, chart); err != nil {
			ec.L.WithError(err).Error("Failed to render Helm chart manifest ", chart.Name)
			return false
		}
	}

	if err := applier.ApplyStack(ctx, ec.clientFactory, &stack, HelmExtensionStackName, HelmExtensionStackName); err != nil {
		fail = true
