...
func (e *Etcd) setupCerts(ctx context.Context) error {
	...
	if err := e.CertManager.EnsureCA("etcd/ca", "etcd-ca", e.Config.CAExpiry); err != nil {
	...
	eg.Go(func() error {
		...
		_, err = e.CertManager.EnsureCertificate(etcdCertReq, uid, e.Config.CertExpiry)
		return err
	})
	...
	eg.Go(func() error {
		...
		_, err := e.CertManager.EnsureCertificate(etcdCertReq, e.uid, e.Config.CertExpiry)
		return err
	})
	...
	eg.Go(func() error {
		...
		_, err := e.CertManager.EnsureCertificate(etcdPeerCertReq, e.uid, e.Config.CertExpiry)
		return err
	})
