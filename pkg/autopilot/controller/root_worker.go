...
func (w *rootWorker) Run(ctx context.Context) error {
	...
	return k8sretry.OnError(wait.Backoff{
		...
		if err := signal.RegisterControllers(ctx, logger, mgr, apdel.NodeControllerDelegate(), &restartTracker, w.cfg.StatusSocketPath, w.cfg.K0sDataDir, true, clusterID, leaderelection.StatusPending); err != nil {
