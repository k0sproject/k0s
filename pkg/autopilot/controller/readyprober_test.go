...
func TestReadyProberContextCanceled(t *testing.T) {
	...
	//nolint:usetesting // Create a context that's initially canceled.
...
func TestReadyProberPrioritizesTargetResolutionFailure2(t *testing.T) {
	var numRequests atomic.Uint32
	hostPort := newTestServer(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		numRequests.Add(1)
		w.WriteHeader(http.StatusInternalServerError)
	...
	cf.K0sClient.PrependReactor("get", "controlnodes", func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
		...
		// This is the "missing" controller. Try to ensure that the
		// goroutine that hit the existing controller has finished and
		// recorded its error.
		for numRequests.Load() < 1 {
			time.Sleep(time.Millisecond)
		}

		// We've seen an HTTP request, wait a little bit longer to increase the
		// likeliness of the other goroutine to have finished.
		time.Sleep(20 * time.Millisecond)

		// Now go ahead and emulate the absence of this controller.
