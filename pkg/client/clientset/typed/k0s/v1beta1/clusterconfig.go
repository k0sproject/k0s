...
func (c *clusterConfigs) List(ctx context.Context, opts v1.ListOptions) (*v1beta1.ClusterConfigList, error) {
	if watchListOptions, hasWatchListOptionsPrepared, watchListOptionsErr := watchlist.PrepareWatchListOptionsFromListOptions(opts); watchListOptionsErr != nil {
		klog.Warningf("Failed preparing watchlist options for clusterconfigs, falling back to the standard LIST semantics, err = %v", watchListOptionsErr)
	} else if hasWatchListOptionsPrepared {
		result, err := c.watchList(ctx, watchListOptions)
		if err == nil {
			consistencydetector.CheckWatchListFromCacheDataConsistencyIfRequested(ctx, "watchlist request for clusterconfigs", c.list, opts, result)
			return result, nil
		}
		klog.Warningf("The watchlist request for clusterconfigs ended with an error, falling back to the standard LIST semantics, err = %v", err)
	}
...
// watchList establishes a watch stream with the server and returns the list of ClusterConfigs
func (c *clusterConfigs) watchList(ctx context.Context, opts v1.ListOptions) (result *v1beta1.ClusterConfigList, err error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
